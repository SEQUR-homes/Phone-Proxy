...
export const handler: ServerlessFunctionSignature = function (
  ...
  if (event.From === privatePhoneNumber) {
    ...
    gather.say('Welcome to the forwarding dashboard. Please enter a phone number followed by the hash key?');
    ...
    twiml.dial({ callerId: proxyPhoneNumber.toString() }, privatePhoneNumber.toString());
    return callback(null, twiml);
  }
};